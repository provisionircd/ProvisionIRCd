...
class Lusers(ircd.Command):
    ...
    def execute(self, client, recv):
        ...
        client.sendraw(self.RPL.LUSERCLIENT, ':There {} {} user{} and {} invisible on {} server{}'.format('are' if len(self.ircd.users) != 1 else 'is', len(self.ircd.users), 's' if len(self.ircd.users) != 1 else '', invisible, servers, 's' if servers != 1 else ''))
