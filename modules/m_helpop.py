...
class Ircdhelp(ircd.Command):
    ...
    def execute(self, client, recv):
        ...
        if recv[1].lower() == 'usercmds':
            ...
            line_queue = []
            width = max([len(name) for c in cmd_list for name in c.command]) + 2
            client.sendraw(292, f':Use /{recv[0].upper()} <command> for more information.')
            client.sendraw(292, ':-')
